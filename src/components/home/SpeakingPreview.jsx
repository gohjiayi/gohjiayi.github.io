...
import TalkCard from '../speaking/TalkCard';
import FadeIn from '../shared/FadeIn';

const PREVIEW_COUNT = 3;

export default function SpeakingPreview({ data }) {
  const talks = Array.isArray(data) ? data : [];

  const latest = useMemo(
    () => [...talks].sort((a, b) => new Date(b.date) - new Date(a.date)).slice(0, PREVIEW_COUNT),
    [talks]
  );
  ...
  return (
    <section id="speaking" className="py-16 md:py-20 scroll-mt-20 bg-gray-50 dark:bg-gray-900">
      <div className="max-w-5xl mx-auto px-6">
        ...
        <div className="space-y-4">
          {latest.map((talk, i) => (
            <FadeIn key={`${talk.title}-${i}`} delay={i * 0.1}>
              <TalkCard talk={talk} />
            </FadeIn>
          ))}
        </div>
        {talks.length > PREVIEW_COUNT && (
          <FadeIn delay={0.3}>
